...
1. The repository publishes a machine-readable **`index.json`** at its root.
2. The plugin fetches `index.json` (raw URL, configurable; default in
   `DEFAULT_MARKET_INDEX_URL`), and shows **bundles** and **individual assets**.
3. Installing writes files into the vault's assets root (default `PaperBell/pandoc/`),
   ...
   installed in `installed.json` there.
