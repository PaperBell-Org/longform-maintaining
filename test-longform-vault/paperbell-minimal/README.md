...
| §10 | **Supplementary Information** step → Figure S1 / Table S1 | `supplementary/` |
...
The **main** draft uses the vault-wide **PaperBell Manuscript** workflow
...
The **supplementary** draft uses the **PaperBell Supplementary** workflow — the same steps with a
**Supplementary Information** step inserted after *Add Zenodo Frontmatter*:

`… → add-zenodo-frontmatter → supplementary-info → write-to-note → Run Pandoc Export`

That step S-numbers the figure/table (Figure S1 / Table S1), retitles the doc
`Supplementary Information for "…"`, drops keywords, and auto-summarizes its section headings into the
abstract (see [docs/PANDOC_EXPORT.md → Supplementary Information](../../../docs/PANDOC_EXPORT.md#supplementary-information)).
Select it for the supplementary draft via Longform's workflow picker.

The **Run Pandoc Export** step has a *Template / preset* dropdown: leave it blank to use the
project's `_longform.template` (`paperbell`), or pick a different downloaded preset per workflow.
...
The final step is the built-in **Run Pandoc Export** step. It shells out to Pandoc to produce the PDF,
reading `acronym`/`date`/`csl`/`template` from the manuscript frontmatter that *Add Zenodo Frontmatter*
already injected. This fixture's step sets **File name** to `{acronym}_{date}`, so it writes
`PBMIN_2026-07-01.pdf`; leaving that option blank would name the PDF after the compiled note instead
(see [docs/PANDOC_EXPORT.md → Naming the PDF](../../../docs/PANDOC_EXPORT.md#naming-the-pdf)).

**Corresponding author.** `_longform.corresponding` lists which creators are corresponding; each such
creator's `email` (here `Song, Shuang` → `song@gea.mpg.de`) is emitted as the `corresponding:` value,
which the LaTeX template prints as the *"Corresponding author: …"* line. Without an `email`, the field
falls back to the `"yes"` flag (which the template would print literally, so add the email).
...
The step itself has **Template / preset** (which downloaded `defaults/*.yaml` to use), **File name**
(the PDF's name, with `{title}`/`{acronym}`/`{date}`/… variables — blank uses the note's name),
**Dry run** (log the command instead of running), and **Open PDF after export**. The Supplementary
draft's frontmatter carries `supplementary: true`, so its figures/tables get S-numbering.
